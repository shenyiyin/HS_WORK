...
#define DEFAULT_UPDATE_MIN_CONN_INTERVAL      600
...
#define DEFAULT_UPDATE_CONN_TIMEOUT           900
...
static const uint8 peerDeviceName[GAP_DEVICE_NAME_LEN]      = "Culligan_TEST ";
...
void update_enter_lowpower(void)
{
	if((judge_bond()==0)&&((simpleBLEState == BLE_STATE_CONNECTED)))
	{
		
		if(GAPCentralRole_TerminateLink(simpleBLEConnHandle))
		{
...
void update_exit_lowpower(void)
{
	if((judge_bond()==0)&&(simpleBLEState != BLE_STATE_CONNECTED))
	{
...
uint8 get_ble_status(void)
{
	return simpleBLEState ;
}
...
int judge_bond(void)
{
		...
		for (i = 0; i < 6; i++) {
				if (store_setting.pair_addr[i] != 0x00) {
						all_zero = 0;
						break;
				}
		}
...
void start_discovery(void)
{
	...
	{
		osal_memset(&store_setting.pair_addr,0x00,6);
...
void SimpleBLECentral_Init( uint8 task_id )
{
    ...
    // Setup the GAP Bond Manager
    {
        ...
        uint8 pairMode = DEFAULT_PAIRING_MODE; //GAPBOND_PAIRING_MODE_INITIATE;//DEFAULT_PAIRING_MODE;    // GAPBOND_PAIRING_MODE_NO_PAIRING
    ...
    for (int i = 0; i < 6; i++)
    {
        simpleBlePeerTarget.addr[i] = store_setting.pair_addr[i];
        AT_LOG("%02x",  store_setting.pair_addr[i]);
    }

    AT_LOG("\n");
//    simpleBlePeerTarget.addr[5]=0x12;
//		 simpleBlePeerTarget.addr[4]=0x34;
//		 simpleBlePeerTarget.addr[3]=0x56;
//		 simpleBlePeerTarget.addr[2]=0x78;
//		 simpleBlePeerTarget.addr[1]=0x90;
//		 simpleBlePeerTarget.addr[0]=0x09;
...
uint16 SimpleBLECentral_ProcessEvent(uint8 task_id, uint16 events)
{
    ...
    if (events & SBC_TERMINATED_CONN)
    {
        if (simpleBLEState == BLE_STATE_CONNECTED) // not connected
        {
            if(GAPCentralRole_TerminateLink(simpleBLEConnHandle))
						{
							osal_start_timerEx(simpleBLETaskId, SBC_TERMINATED_CONN, 200);
						}
        }
               ...
               ( pMsg->msg.errorRsp.reqOpcode == ATT_HANDLE_VALUE_NOTI ) ) )
    {
        ...
        else
        {
            //LOG( "Read Ntf Len : %d\r\n", pMsg->msg.handleValueNoti.len);
						...
						{
								parseProtocol(pMsg->msg.handleValueNoti.value[i]);
...
//    static int try_num = 0;
    ...
    case GAP_DEVICE_INIT_DONE_EVENT:
    {
        ...
        osal_start_timerEx(simpleBLETaskId,CENTRAL_INIT_DONE_EVT,10);
    }
    break;
...
//            AT_LOG("[simpleBLEDevList[i].LocalName.Type%d] ",simpleBLEDevList[i].LocalName.Type);
            if( simpleBLEDevList[i].LocalName.Type )
            {
               // AT_LOG("[Name 0x%02X  LENGTH=%d]:",simpleBLEDevList[i].LocalName.Type,simpleBLEDevList[i].LocalName.Length);
                char name[31];
    ...
    case GAP_LINK_ESTABLISHED_EVENT:
    {
        ...
        if ( pEvent->gap.hdr.status == SUCCESS )
        {
            ...
            s_linkActiveDuration=osal_getClock();
					  ...
					  osal_memcpy(store_setting.pair_addr,pEvent->linkCmpl.devAddr,6);
						save_setting(&store_setting);
						findDevByName=FALSE;
            //--------------------------------------------------------------------------------------
            //MTU Size Exchange
            ...
            HCI_ReadRemoteVersionInfoCmd(simpleBLEConnHandle);
						osal_start_timerEx(User_App_TaskID, user_notifyenable_event_id, 450);
...
bool compareUUID(const uint8_t* advertData, uint8_t dataLength)
{
	  char targetUUID[] = {'H','S','D', 'Q', '.', 'C', 'o', '.', 'L', 't', 'd', ' ', 'c', 'l', 'g'};
    if (dataLength < sizeof(targetUUID))
    {
        return false; // 数据长度小于目标UUID长度，无法比较
    }

    for (int i = 0; i < sizeof(targetUUID); i++)
    {
        if (advertData[i + 2] != targetUUID[i]) // 从第7个字节开始比较UUID字段
        {
            return false; // UUID字段不匹配
        }
    }

    return true; // UUID字段匹配
}
    ...
    if ( simpleBLEScanRes > 0 )
    {
        for(uint8_t i = 0; i< simpleBLEScanRes; i++)
        {
            if(findDevByName)
            {
								...
								int length = simpleBLEDevList[i].ManufactureData.Length;
								
								 if (compareUUID(simpleBLEDevList[i].ManufactureData.Value,simpleBLEDevList[i].ManufactureData.Length)) {
											Index = i;
											LOG("\nlength =0x%x\n ", length);
											LOG("0x%x ", simpleBLEDevList[i].ManufactureData.Value[0]);
											break;
									}
