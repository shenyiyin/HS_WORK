...
// GAP GATT Attributes
static uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = "DIAL-dd40 ";
/*********************************************************************
    LOCAL FUNCTIONS
*/
...
void SimpleBLEPeripheral_Init( uint8 task_id )
{
    ...
    {
        uint16 advInt = 300;//2400;//1600;//1600;//800;//1600;   // actual time = advInt * 625us
        ...
        for(int i=0; i<notifyPktNum; i++)
        {
            ...
            else
            {
                ...
                break;
						...
						status= simpleProfile_Notify(SIMPLEPROFILE_CHAR5,ATT_GetCurrentMTUSize(0)-3,notifyBuf);
...
{
		uint8_t test=0;
		GAPRole_SetParameter( GAPROLE_ADVERT_ENABLED, sizeof( test ), &test );
}
