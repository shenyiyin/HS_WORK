      ...
      user_app_check_keys(now);
			...
			if(temp_angle!=0xFFF)
			{
				  ...
				  if(angle==0xFFF)
					...
					else if(((temp_angle-angle)>200)||((temp_angle-angle)<-200))
					{
						angle=temp_angle;
					}
					else if((temp_angle-angle)>10)
					{
						DC.percent += 1; // Decrement the angle if a falling edge is detected on E2
						angle=temp_angle;
					}
					else if((temp_angle-angle)<-10)
					{
						DC.percent -= 1; // Increment the angle if a falling edge is detected on E1
						angle=temp_angle;
					}
			}
